 ...
 */
public interface UserFieldGroupRepository extends JpaRepository<UserFieldGroup, Long> {
    /**
     * Find by id user.
     *
     * @param id the id
	...
	List <UserFieldGroup>   findByOrgId(String  orgId);
	
	//查询表单的字段
	@Query(value = "select * from t_user_field  WHERE flow_id =?1 AND p_id is null",nativeQuery = true)
	List <UserFieldGroup>   findByflowId(String  flowId);
	
	//查询表单的字段
	@Query(value = "select * from t_user_field  WHERE  p_id =?1",nativeQuery = true)
	List <UserFieldGroup>   findByPid(String  pid);
