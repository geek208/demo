...
<div class="container">
   ...
   <el-form ref="postForm" :v-model = "postForm" class="form-container" label-width="100px" >
            ...
            <input type="text hidden" hidden :v-model ="postForm.amount" />
            <input type="text hidden" hidden :v-model ="postForm.price" />
            <!-- <input type="text hidden" hidden :v-model ="postForm.month" /> -->

            <div><p class="selectpan">选择会员计划</p></div>
            <div class="plangroup">
                 <!-- <div class="itme" v-for="item in list" :key="item.id">
                     {{item.name}}  测试
                 </div> -->
             <div @click="changeplan(item.id,item.price,item.name,item.level)" :class="item.id ==id?'planitem active':'planitem'" 
                    v-for="item in list" :key="item.id">
                    <div class="planprice"><span>¥{{item.price}}</span><p>/月</p></div>
            ...
            <div class="jiesuan">
                ...
                <div><el-input class="attch" v-model="postForm.attch" /></div>
...
import { packageListAll } from '@/api/package'
...
export default {
  ...
  methods:{
    ...
    getList() {
      ...
      //debugger
      packageListAll(this.listQuery).then(response => {
    ...
    submitForm(){
       ...
       addOrder(formData).then(response => {
        ...
        //console.log(obj.total+' =====  '+obj.accountId+'======='+obj.packageId)
        this.$router.push({ path: "bill", query: {id:obj.id}})
        }).catch(err => {
            this.loading =true
            console.log(err)
        }).finally(() => {
    ...
    changeplan(id,price,name,level){
      this.id=id;
      this.postForm.packageId =id;
      //折扣后的价格
      this.postForm.amount = price;
      //标价
      this.postForm.price = price;
      this.postForm.packageName =name;
      this.postForm.level = level;
      const v = this.postForm.month*price;
      //实际支付总价
      this.postForm.total = v.toFixed(2);
      console.log(this.postForm.total);
    },
    changemonth(month){
    ...
    changepay(paytype){
     ...
     console.log(this.postForm.payType);
    }
  }
}
</script>
